Create Codex pet row `running-right` for `brocore`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Compact full-body friendly tech mascot, energetic big-bro teammate, confident pro developer vibe, warm grin, hoodie or small utility vest, tiny headset, subtle gamepad-and-code inspired shapes as non-readable props, bright teal, electric blue, warm amber accents, helpful brother personality, no text or logos.. Preserve silhouette, face, palette, material, proportions, markings, and props.
